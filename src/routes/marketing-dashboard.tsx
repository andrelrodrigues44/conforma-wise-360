...
import { BarChart3, Bot, Check, CheckCircle2, FileText, Flame, LogOut, Megaphone, MessageSquareText, Plus, RefreshCw, Save, Send, Target, Users, X, Eye, ThumbsDown, Image as ImageIcon, Upload, Trash2 } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Textarea } from "@/components/ui/textarea";

type Row = { [k: string]: unknown } & Partial<Record<"agendado_para" | "assunto" | "campaign_id" | "canal" | "cargo" | "created_at" | "criativo_brief" | "criativo_url" | "criativo_urls" | "criativo_alt" | "cta" | "data_publicacao" | "email" | "empresa" | "enviado_em" | "etapa" | "formato" | "id" | "interesse" | "lead_id" | "legenda" | "linha_comercial" | "mensagem" | "nome" | "objetivo" | "periodo_fim" | "periodo_inicio" | "porte" | "proxima_acao" | "proxima_acao_em" | "score" | "segmento" | "status" | "telefone" | "temperatura" | "titulo" | "ultimo_contato_em" | "unidades", unknown>>;
...
// Carrossel guarda todas as URLs em criativo_urls (JSONB); post de imagem
// única continua só com criativo_url. Uma função só de leitura pras duas
// telas (cartão da lista e modal de revisão) tratarem os dois formatos
// igual, sem duplicar a lógica de fallback.
function imageList(row: Row): string[] {
  const raw = row.criativo_urls;
  if (Array.isArray(raw)) {
    const list = raw.filter((url): url is string => typeof url === "string" && url.length > 0);
    if (list.length > 0) return list;
  }
  const single = text(row.criativo_url);
  return single ? [single] : [];
}
const moneyLabel = (value: unknown) => value === "consultoria" ? "Consultoria" : value === "plataforma" ? "Plataforma" : "Ambos";
const statusLabel = (value: unknown) => ({ revisar: "Pendente de revisão", aprovado: "Aprovado", rejeitado: "Rejeitado / ajuste necessário", rascunho: "Rascunho / ajuste necessário", agendado: "Agendado", publicado: "Publicado", arquivado: "Arquivado" } as Record<string, string>)[text(value)] || text(value);
...
function MarketingDashboardPage() {
  ...
  const [publishingRow, setPublishingRow] = useState<Row | null>(null);
  ...
  async function mutate(method: "POST" | "PATCH", table: string, id: string | undefined, values: Row, close = true) { setLoading(true); setError(""); try { const response = await fetch("/api/admin/manage", { method, credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ table, id, data: values }) }); const json = await response.json().catch(() => ({})); if (!response.ok) throw new Error(json.error || "Não foi possível salvar."); if (close) { setEditing(null); setCreating(null); setReviewing(null); setPublishingRow(null); } await loadDashboard(); } catch (e) { setError(e instanceof Error ? e.message : "Erro ao salvar."); } finally { setLoading(false); } }
  async function deleteContent(item: Row) {
    if (!window.confirm(`Excluir "${text(item.titulo)}"? Não tem como desfazer.`)) return;
    setLoading(true); setError("");
    try {
      const response = await fetch(`/api/admin/manage?table=marketing_contents&id=${encodeURIComponent(text(item.id))}`, { method: "DELETE", credentials: "include" });
      const json = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(json.error || "Não foi possível excluir.");
      await loadDashboard();
    } catch (e) {
  ...
  // Upload manual do criativo (imagem feita fora, não mais gerada pelo
  // pipeline) -- sobe pro Storage via /api/admin/upload e já grava a URL
  // no conteúdo. Múltiplos arquivos = carrossel, na ordem selecionada.
  async function uploadAnexo(item: Row, fileList: FileList | null) {
    if (!fileList || !fileList.length) return;
    setLoading(true); setError("");
    try {
      const form = new FormData();
      Array.from(fileList).forEach((file) => form.append("files", file));
      const response = await fetch("/api/admin/upload", { method: "POST", credentials: "include", body: form });
      const json = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(json.error || "Não foi possível enviar a imagem.");
      const urls = json.urls as string[];
      await mutate("PATCH", "marketing_contents", text(item.id), { criativo_url: urls[0], criativo_urls: urls }, false);
    } catch (e) {
  ...
  const reviewContents = (data?.contents ?? []).filter((content) => text(content.status) === "revisar");
  const ideas = ["Publicar conteúdo técnico de autoridade sobre conformidade legal e redução de riscos.", "Criar CTA direto para demonstração da plataforma Conforma360.", "Priorizar follow-up dos HOT leads antes de gerar novos contatos."];
  if ((stats?.reviewContents ?? 0) > 0) ideas.unshift(`Revisar ${stats?.reviewContents} conteúdo(s) pendente(s) antes da próxima publicação.`);
  const suggestions = ideas.slice(0, 4);

  return <div className="min-h-screen bg-[#f5f7f6] text-graphite"><header className="sticky top-0 z-30 border-b border-border bg-white/95 backdrop-blur"><div className="mx-auto flex h-16 max-w-[1500px] items-center justify-between px-5 lg:px-8"><div className="flex items-center gap-3"><div className="grid h-9 w-9 place-items-center rounded-xl bg-primary text-white"><Bot className="h-5 w-5" /></div><div><p className="text-[10px] font-extrabold tracking-[0.18em] text-primary">CONFORMA360</p><p className="font-bold">Marketing AI</p></div></div><div className="flex items-center gap-2"><Button variant="outline" size="sm" onClick={() => void loadDashboard()} disabled={loading}><RefreshCw className={`mr-2 h-4 w-4 ${loading ? "animate-spin" : ""}`} />Atualizar</Button><Button variant="ghost" size="sm" onClick={() => void logout()}><LogOut className="mr-2 h-4 w-4" />Sair</Button></div></div></header><main className="mx-auto max-w-[1500px] px-5 py-8 lg:px-8"><div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between"><div><p className="text-xs font-extrabold tracking-[0.2em] text-primary">CENTRO DE COMANDO</p><h1 className="mt-2 text-3xl font-extrabold tracking-tight sm:text-4xl">Marketing & Sales Intelligence</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">Gestão, sugestões, revisão e aprovação da operação em um único lugar.</p></div><div className="flex items-center gap-2 rounded-2xl border border-primary/15 bg-white px-4 py-3 text-xs font-semibold"><span className="h-2 w-2 rounded-full bg-primary" />Sales Engine conectado</div></div><nav className="mt-8 flex gap-1 overflow-x-auto rounded-2xl border border-border bg-white p-1.5 shadow-soft">{tabs.map(([id, label, Icon]) => <button key={id} onClick={() => { setTab(id); setEditing(null); setCreating(null); setReviewing(null); }} className={`relative flex shrink-0 items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-bold transition ${tab === id ? "bg-primary text-white" : "text-muted-foreground hover:bg-surface hover:text-graphite"}`}><Icon className="h-4 w-4" />{label}{id === "conteudos" && reviewContents.length > 0 && <span className={`ml-1 rounded-full px-1.5 py-0.5 text-[10px] ${tab === id ? "bg-white/20 text-white" : "bg-primary text-white"}`}>{reviewContents.length}</span>}</button>)}</nav>{error && <div className="mt-5 flex items-center justify-between rounded-2xl border border-destructive/20 bg-destructive/5 p-4 text-sm text-destructive"><span>{error}</span><button onClick={() => setError("")}><X className="h-4 w-4" /></button></div>}
{tab === "visao-geral" && <Overview stats={stats} hotLeads={hotLeads} reviewContents={reviewContents} suggestions={suggestions} onTab={setTab} onReview={setReviewing} />}
...
{tab === "conteudos" && <><div className="mb-5 flex items-center justify-between gap-3"><div><p className="text-sm font-semibold text-muted-foreground">Fila de aprovação</p><p className="text-xs text-muted-foreground">A IA prepara o texto. Você anexa a arte (feita fora) e decide o que pode avançar.</p></div><Button onClick={() => setCreating("content")}><Plus className="mr-2 h-4 w-4" />Novo conteúdo</Button></div><ContentSection items={data?.contents ?? []} onReview={setReviewing} onEdit={(row) => setEditing({ table: "marketing_contents", row })} onPublish={setPublishingRow} onUpload={uploadAnexo} onDelete={deleteContent} /></>}
...
function CreativeGallery({ urls, alt, className }: { urls: string[]; alt: string; className?: string }) { if (urls.length === 0) return null; return <div className={className}><div className="flex gap-2 overflow-x-auto pb-1">{urls.map((url, index) => <div key={url} className="relative shrink-0 overflow-hidden rounded-2xl border border-border bg-surface" style={{ width: urls.length > 1 ? 220 : "100%" }}><img src={url} alt={`${alt} — slide ${index + 1}`} className="block h-auto max-h-[420px] w-full object-contain" loading="lazy" />{urls.length > 1 && <span className="absolute left-2 top-2 rounded-full bg-black/60 px-2 py-0.5 text-[10px] font-bold text-white">{index + 1}/{urls.length}</span>}</div>)}</div></div>; }

function ContentSection({ items, onReview, onEdit, onPublish, onUpload, onDelete }: { items: Row[]; onReview: (row: Row) => void; onEdit: (row: Row) => void; onPublish: (row: Row) => void; onUpload: (row: Row, files: FileList | null) => void; onDelete: (row: Row) => void }) { return <section className="mt-6 rounded-3xl border border-border bg-white p-6 shadow-soft"><div className="flex items-center justify-between"><div><p className="text-xs font-extrabold tracking-[0.16em] text-primary">GESTÃO</p><h2 className="mt-1 text-2xl font-extrabold">Conteúdos</h2></div><span className="text-xs font-semibold text-muted-foreground">{items.length} registro(s)</span></div><div className="mt-6 divide-y divide-border">{items.map((item) => <div key={text(item.id)} className="flex items-start justify-between gap-4 py-5"><div className="min-w-0 flex-1"><CreativeGallery urls={imageList(item)} alt={text(item.criativo_alt) || text(item.titulo)} className="mb-4" /><p className="font-bold">{text(item.titulo)}</p><p className="mt-1 text-xs text-muted-foreground">{text(item.canal)} · {text(item.formato)} · {moneyLabel(item.linha_comercial)}</p><p className={`mt-2 text-xs font-bold ${item.status === "revisar" ? "text-amber-600" : item.status === "aprovado" ? "text-primary" : "text-muted-foreground"}`}>{statusLabel(item.status)} · {text(item.data_publicacao) || "sem data"}</p></div><div className="flex shrink-0 flex-col items-end gap-2"><div className="flex flex-wrap justify-end gap-2"><label className="inline-flex h-9 cursor-pointer items-center rounded-xl border border-border bg-white px-3 text-xs font-bold text-graphite hover:bg-surface"><Upload className="mr-2 h-4 w-4" />{imageList(item).length > 0 ? "Trocar imagem(ns)" : "Anexar imagem(ns)"}<input type="file" accept="image/*" multiple className="hidden" onChange={(e) => { onUpload(item, e.target.files); e.target.value = ""; }} /></label><Button variant="outline" size="sm" onClick={() => onReview(item)}><Eye className="mr-2 h-4 w-4" />{item.status === "revisar" ? "Revisar" : "Visualizar"}</Button><Button variant="ghost" size="sm" onClick={() => onEdit(item)}>Editar</Button><Button variant="ghost" size="sm" className="text-destructive hover:bg-destructive/10 hover:text-destructive" onClick={() => onDelete(item)}><Trash2 className="mr-2 h-4 w-4" />Excluir</Button></div>{item.status === "aprovado" && <Button size="sm" onClick={() => onPublish(item)}><Send className="mr-2 h-4 w-4" />Publicar</Button>}</div></div>)}{items.length === 0 && <div className="py-12 text-center text-sm text-muted-foreground">Nenhum conteúdo criado.</div>}</div></section>; }

function DataSection({ title, items, empty, onEdit, render }: { title: string; items: Row[]; empty: string; onEdit: (row: Row) => void; render: (row: Row) => ReactNode }) { return <section className="mt-6 rounded-3xl border border-border bg-white p-6 shadow-soft"><div className="flex items-center justify-between"><div><p className="text-xs font-extrabold tracking-[0.16em] text-primary">GESTÃO</p><h2 className="mt-1 text-2xl font-extrabold">{title}</h2></div><span className="text-xs font-semibold text-muted-foreground">{items.length} registro(s)</span></div><div className="mt-6 divide-y divide-border">{items.map((item) => <div key={text(item.id)} className="flex items-start justify-between gap-4 py-4"><div className="min-w-0 flex-1">{render(item)}</div><Button variant="outline" size="sm" onClick={() => onEdit(item)}>Editar</Button></div>)}{items.length === 0 && <div className="py-12 text-center text-sm text-muted-foreground">{empty}</div>}</div></section>; }

function ReviewModal({ row, saving, onCancel, onApprove, onReject }: { row: Row; saving: boolean; onCancel: () => void; onApprove: () => void; onReject: () => void }) { const urls = imageList(row); return <div className="fixed inset-0 z-50 overflow-y-auto bg-black/40 p-5"><div className="mx-auto mt-6 max-w-4xl rounded-3xl border border-border bg-white p-6 shadow-elevated"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-extrabold tracking-[0.16em] text-primary">APROVAÇÃO DE CONTEÚDO</p><h2 className="mt-1 text-2xl font-extrabold">{text(row.titulo)}</h2><p className="mt-1 text-sm text-muted-foreground">{text(row.canal)} · {text(row.formato)} · {moneyLabel(row.linha_comercial)}</p></div><Button variant="ghost" onClick={onCancel}><X /></Button></div><div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]"><div><p className="mb-2 flex items-center gap-2 text-xs font-extrabold uppercase tracking-[0.12em] text-primary"><ImageIcon className="h-4 w-4" />Prévia do criativo{urls.length > 1 && <span className="ml-1 font-normal normal-case text-muted-foreground">· carrossel, {urls.length} slides</span>}</p>{urls.length > 0 ? <CreativeGallery urls={urls} alt={text(row.criativo_alt) || text(row.titulo)} /> : <div className="grid min-h-[360px] place-items-center rounded-2xl border border-dashed border-border bg-surface p-8 text-center text-sm text-muted-foreground">Nenhuma arte foi gerada para este conteúdo.</div>}{urls.length === 1 && <a className="mt-2 block text-center text-xs font-semibold text-primary hover:underline" href={urls[0]} target="_blank" rel="noreferrer">Abrir prévia em nova aba</a>}</div><div className="space-y-5"><ReviewBlock label="Legenda" value={row.legenda} /><ReviewBlock label="CTA" value={row.cta} /><ReviewBlock label="Brief do criativo" value={row.criativo_brief} /><div className="rounded-2xl bg-surface p-4 text-sm"><span className="font-bold">Status atual: </span>{statusLabel(row.status)}</div></div></div><div className="mt-7 flex flex-col-reverse gap-2 sm:flex-row sm:justify-between"><Button variant="outline" onClick={onCancel}>Voltar</Button><div className="flex gap-2"><Button variant="outline" onClick={onReject} disabled={saving}><ThumbsDown className="mr-2 h-4 w-4" />Rejeitar / pedir ajuste</Button><Button onClick={onApprove} disabled={saving}><Check className="mr-2 h-4 w-4" />{saving ? "Processando…" : "Aprovar conteúdo"}</Button></div></div></div></div>; }
function ReviewBlock({ label, value }: { label: string; value: unknown }) { return <div><p className="mb-2 text-xs font-extrabold uppercase tracking-[0.12em] text-primary">{label}</p><div className="whitespace-pre-wrap rounded-2xl border border-border bg-white p-4 text-sm leading-6">{text(value) || "Não informado."}</div></div>; }
function Editor({ table, row, saving, onCancel, onSave }: { table: string; row: Row; saving: boolean; onCancel: () => void; onSave: (values: Row) => void }) { const fields: Array<[string, string]> = table === "leads_site" ? [["nome", "Nome"], ["empresa", "Empresa"], ["cargo", "Cargo"], ["linha_comercial", "Linha comercial"], ["interesse", "Interesse"], ["segmento", "Segmento"], ["score", "Score"], ["temperatura", "Temperatura"], ["etapa", "Etapa"], ["status", "Status"], ["proxima_acao", "Próxima ação"]] : table === "marketing_contents" ? [["titulo", "Título"], ["canal", "Canal"], ["formato", "Formato"], ["linha_comercial", "Linha comercial"], ["cta", "CTA"], ["data_publicacao", "Data de publicação"], ["status", "Status"], ["criativo_url", "URL da imagem (cole o link após subir no Storage)"], ["legenda", "Legenda"], ["criativo_brief", "Brief do criativo"]] : table === "marketing_campaigns" ? [["nome", "Nome"], ["objetivo", "Objetivo"], ["linha_comercial", "Linha comercial"], ["segmento", "Segmento"], ["periodo_inicio", "Início"], ["periodo_fim", "Fim"], ["status", "Status"]] : [["etapa", "Etapa"], ["canal", "Canal"], ["assunto", "Assunto"], ["mensagem", "Mensagem"], ["agendado_para", "Agendado para"], ["status", "Status"]]; const [values, setValues] = useState<Row>(() => Object.fromEntries(fields.map(([key]) => [key, row[key] ?? ""]))); return <Modal title="Editar registro" saving={saving} onCancel={onCancel} onSave={() => onSave(values)}><div className="grid gap-4 sm:grid-cols-2">{fields.map(([key, label]) => <label key={key} className={key === "mensagem" || key === "legenda" || key === "criativo_brief" ? "sm:col-span-2" : ""}><span className="mb-1.5 block text-xs font-bold text-muted-foreground">{label}</span>{["mensagem", "legenda", "criativo_brief"].includes(key) ? <Textarea rows={key === "legenda" ? 9 : 6} value={text(values[key])} onChange={(e) => setValues({ ...values, [key]: e.target.value })} /> : <Input value={text(values[key])} onChange={(e) => setValues({ ...values, [key]: e.target.value })} />}</label>)}</div></Modal>; }
function CreateForm({ title, fields, initial, saving, onCancel, onCreate }: { title: string; fields: Array<[string, string]>; initial: Row; saving: boolean; onCancel: () => void; onCreate: (values: Row) => void }) { const [values, setValues] = useState<Row>(initial); return <Modal title={title} saving={saving} onCancel={onCancel} onSave={() => onCreate(values)}><div className="grid gap-4 sm:grid-cols-2">{fields.map(([key, label]) => <label key={key}><span className="mb-1.5 block text-xs font-bold text-muted-foreground">{label}</span><Input value={text(values[key])} onChange={(e) => setValues({ ...values, [key]: e.target.value })} /></label>)}</div></Modal>; }
function Modal({ title, saving, onCancel, onSave, children }: { title: string; saving: boolean; onCancel: () => void; onSave: () => void; children: ReactNode }) { return <div className="fixed inset-0 z-50 overflow-y-auto bg-black/40 p-5"><div className="mx-auto mt-10 max-w-2xl rounded-3xl border border-border bg-white p-6 shadow-elevated"><div className="flex items-center justify-between"><div><p className="text-xs font-extrabold tracking-[0.16em] text-primary">ADMINISTRAÇÃO</p><h2 className="mt-1 text-2xl font-extrabold">{title}</h2></div><Button variant="ghost" onClick={onCancel}><X /></Button></div><div className="mt-6">{children}</div><div className="mt-6 flex justify-end gap-2"><Button variant="outline" onClick={onCancel}>Cancelar</Button><Button onClick={onSave} disabled={saving}><Save className="mr-2 h-4 w-4" />{saving ? "Salvando…" : "Salvar"}</Button></div></div></div>; }

// Canal do Buffer -- carregado sob demanda (GET /api/admin/buffer lista
// as organizações e seus canais conectados). Publica de verdade (não
// como rascunho): action "schedule" com dueAt = agora faz o Buffer
// tratar como um post agendado pro momento atual, que ele processa e
// publica sozinho -- diferente de action "draft", que só salva um
// rascunho dentro do Buffer, exigindo uma ação manual extra lá dentro.
...
function PublishModal({ row, onCancel, onPublished }: { row: Row; onCancel: () => void; onPublished: () => void }) {
  ...
  useEffect(() => {
    let active = true;
    fetch("/api/admin/buffer", { credentials: "include" })
      .then((response) => response.json())
      .then((json) => {
        if (!active) return;
        if (json?.error) throw new Error(json.error);
        type OrgWithChannels = { channels?: BufferChannel[] };
  ...
  const carouselUrls = imageList(row);
