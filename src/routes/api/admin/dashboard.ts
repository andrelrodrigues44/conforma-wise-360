...
function sessionValid(request: Request) {
  ...
  try {
    return value === "admin" && timingSafeEqual(Buffer.from(signature), Buffer.from(expected));
  } catch {
    return false;
  }
}
    ...
    handlers: {
      GET: async ({ request }) => {
        ...
        try {
          ...
          const countBy = (items: any[], key: string) => items.reduce((acc, item) => {
            const value = item[key] || "sem_classificacao";
          ...
          const stats = {
            ...
            byLine: countBy(leads, "linha_comercial"),
            byStage: countBy(leads, "etapa"),
            byTemperature: countBy(leads, "temperatura"),
          };

          return new Response(JSON.stringify({ stats, leads, followups, contents, campaigns, activities }), {
